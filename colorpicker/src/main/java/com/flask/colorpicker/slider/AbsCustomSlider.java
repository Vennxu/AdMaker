...
public abstract class AbsCustomSlider extends View {
	...
	protected void createBitmaps() {
		...
		bar = Bitmap.createBitmap(barHeight, getHeight(), Bitmap.Config.ARGB_8888);
	...
	protected void onDraw(Canvas canvas) {
		...
		if (bar != null && bitmapCanvas != null) {
			...
			bitmapCanvas.drawBitmap(bar, (getWidth()-bar.getWidth())/2, (getHeight() - bar.getHeight()) / 2, null);

			float x = getWidth() / 2f;
			float y = handleRadius + value * (getHeight() - handleRadius * 2);
	...
	public boolean onTouchEvent(MotionEvent event) {
		switch (event.getAction()) {
			...
			case MotionEvent.ACTION_MOVE: {
				value = (event.getY() - barOffsetX) / bar.getHeight();
